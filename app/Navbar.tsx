...
>
     <nav className="w-full flex items-center justify-between px-4 sm:px-6 md:px-8 py-3 md:py-4">
        ...
        <ul className="hidden md:flex gap-8 text-white font-medium items-center">
          {navItems.map((item) => (
            <motion.li key={item.id} whileHover={{ scale: 1.06 }}>
              <a
                href={item.href}
        ...
        {menuOpen && (
          ...
          >
              ...
              <li key={item.id}>
                <a
                  ...
                  className={`block text-center py-3 ${
                    active === item.id ? "text-blue-200" : "text-white/95"
                  }`}
                >
                  {item.label}
                </a>
            ...
            <li>
              <a
                href="#contact"
                className="block text-center bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-3 rounded-lg font-semibold"
              >
                Request a Quote
              </a>
            </li>
